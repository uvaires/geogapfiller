...
def predicted_img(inputdir: str, outputdir: str, base_dates: list, window=15, interval=1, n_jobs=-1):
    """
    Fill the gaps in the geospatial rasters using a median-based approach
    ...
    :param window: window size for median filling
    ...
    # Fill the missing values in the image layers using median-based filling
...
def predict_daily_imgs(stack_rasters, base_dates, interval=1, window=15, n_jobs=-1):
    ...
    filled_arr = _median_filling(original_arr, date_range, window=window, n_jobs=n_jobs)

    return filled_arr, date_range


def _median_filling(stacked_images, base_dates, window=15, n_jobs=-1):
    ...
    def fill_missing_for_index(i, j, raster_values, base_dates):
        ...
        for index_ii, raster_value in enumerate(raster_values_filled):
            if np.isnan(raster_value):
                ...
                # If there are valid values in the window, use their median
                if valid_indices:
                    valid_values = raster_values_filled[valid_indices]
                    median_value = np.nanmedian(valid_values)

                    # Clip values to stay within range [-1, 1] (if applicable)
                    median_value = np.clip(median_value, -1, 1)
...
def _export_img_daily(outputdir, img_predicted, img_profile, data_range):
    for layer_idx, (layer_data, current_dates) in enumerate(zip(img_predicted, data_range)):
        print(current_dates)
        dir_output_date = os.path.join(outputdir, 'predicted_img', 'median_pred')
