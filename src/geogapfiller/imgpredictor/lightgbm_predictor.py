...
def predicted_img(inputdir: str, outputdir: str, base_dates: list, window=15, interval=1, n_jobs=-1):
    """
    Fill the gaps in the geospatial rasters using a LightGBM regression model
    ...
    :param window: window size for LightGBM filling
    ...
    # Fill the missing values in the image layers using LightGBM filling
...
def predict_daily_imgs(stack_rasters, base_dates, interval=1, window=15, n_jobs=-1):
    ...
    # Use LightGBM to fill the gaps
    filled_arr = _lightgbm_filling(original_arr, date_range, window=window, n_jobs=n_jobs)

    return filled_arr, date_range


def _lightgbm_filling(stacked_images, base_dates, window=15, n_jobs=-1):
    filled_arr = stacked_images.copy()

    def lgb_model_fit(X, y):
    ...
    def lgb_predict(model, X_pred):
        # Predict the missing value using the trained LightGBM model
...
def _export_img_daily(outputdir, img_predicted, img_profile, data_range):
    for layer_idx, (layer_data, current_dates) in enumerate(zip(img_predicted, data_range)):
        print(current_dates)
        dir_output_date = os.path.join(outputdir, 'predicted_img', 'lightgbm_pred')
